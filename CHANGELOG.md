...
* `dev/nrepl` template: Upgraded `sci.nrepl` dependency to include the local `describe` op, improving compatibility with `clojure-mcp`
* `dev/nrepl` template: Renamed `nrepl-server.clj` to `nrepl-proxy.clj` for clarity
